    ...
    public function Eligilibility($request=null)
    {
        if($request->gst_phy > 5 || $request->gst_math >6)
        {
            if (!empty($request['el_subjects'])) {
                $request->merge(['el_subjects' => array_merge($request['el_subjects'],['Computer Science and Engineering'])]);
            }
            else{
                $request->merge(['el_subjects'=>['Computer Science and Engineering']]);
            }
        }
